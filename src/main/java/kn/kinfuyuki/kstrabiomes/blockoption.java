...
public class blockoption implements OptionsComponent {
	...
	private blocksearchfieldcomponent it;

	public blockoption(String translationKey) {
		this.searched=false;
	...
	public blockoption(String translationKey,blocksearchfieldcomponent s) {
		this.searched=true;
		this.name = translationKey;
		this.resetButton = (new ButtonElement(0, 0, 0, 20, 20, "")).setTextures("minecraft:gui/misc/icon_reset", "minecraft:gui/misc/icon_reset_highlighted", "minecraft:gui/misc/icon_reset");
		it=s;
	...
	public void tick() {
		if (!name.equals(prevname)){
			...
			Integer i=Blocks.keyToIdMap.get(name);
			if (i!=null) {
	...
	public void render(int x, int y, int width, int relativeMouseX, int relativeMouseY) {
		...
		if (current==null)icon.render(defaultStack,x+width-32,y);
		else {
			icon.drawGuiTexture(mc.textureManager,x+width-32,y,getHeight(),getHeight(),
			TextureRegistry.getTexture(current.namespaceId()).getImageSource());
		}
	}
